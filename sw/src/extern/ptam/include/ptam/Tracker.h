...
{
public:
    Tracker(Context *ctx, /*CVD::ImageRef irVideoSize,*/ const ATANCamera &c, Map &m, MapMaker &mm);
    
    // TrackFrame is the main working part of the tracker: call this every frame.
    void TrackFrame(CVD::BasicImage<CVD::byte> &imFrame, bool bDraw);
    ...
    std::vector<CVD::ImageRef> const& GetTrackingCorners() const;
    std::list<Trail> const& GetTrackingTrails() const;
