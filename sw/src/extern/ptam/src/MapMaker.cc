...
MapMaker::MapMaker(Context *ctx, Map& m, const ATANCamera &cam)
: mContext(ctx), mMap(m), mCamera(cam)
{
...
bool MapMaker::InitFromStereo(KeyFrame &kF,
                              ...
                              SE3<> &se3TrackerPose)
{
    mdWiggleScale = mContext->ptamMapMakerWiggleScale; // Cache this for the new map.
    ...
    KeyFrame *pkFirst = new KeyFrame(mContext);
    KeyFrame *pkSecond = new KeyFrame(mContext);
    *pkFirst = kF;
    *pkSecond = kS;
...
{
    KeyFrame *pK = new KeyFrame(mContext);
    *pK = k;
...
bool MapMaker::NeedNewKeyFrame(KeyFrame &kCurrent)
{
    ...
    if(dDist > mContext->ptamMapMakerMaxKFDistWiggleMult * mdWiggleScaleDepthNormalized)
        return true;
    return false;
}
...
{
    Bundle b(mContext, mCamera);   // Our bundle adjuster
...
SE3<> MapMaker::CalcPlaneAligner()
{
    ...
    int nRansacs = mContext->ptamMapMakerPlaneAlignerRansacs;
    Vector<3> v3BestMean;
