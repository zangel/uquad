...
#define cout if(mContext->mgvnBundleCout) cout
...
Bundle::Bundle(Context *ctx, const ATANCamera &TCam)
: mContext(ctx)
, mCamera(TCam)
{
    mnCamsToUpdate = 0;
    ...
    while(!mbConverged  && !mbHitMaxIterations && !*pbAbortSignal)
    {
        ...
        switch(mContext->ptamBundleMEstimator)
        {
            ...
            default:
                ...
                //*gvsMEstimator = "Tukey";
                mContext->ptamBundleMEstimator = kMEstimatorTukey;
                bNoError = Do_LM_Step<Tukey>(pbAbortSignal);
                break;
        }
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal)
{
    ...
    const double dMinSigmaSquared = mContext->ptamBundleMinTukeySigma * mContext->ptamBundleMinTukeySigma;
    ...
    while(dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations && !*pbAbortSignal)
    {
        ...
        if(dSumSquaredUpdate< mContext->ptamBundleUpdateConvergenceLimit)
            mbConverged = true;
        ...
        mnCounter++;
        if(mnCounter >= mContext->ptamBundleMaxIterations)
