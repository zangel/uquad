...
Tracker::Tracker(Context *ctx, /*ImageRef irVideoSize,*/ const ATANCamera &c, Map &m, MapMaker &mm)
: mContext(ctx)
, mCurrentKF(mContext)
, mMap(m)
, mMapMaker(mm)
, mCamera(c)
, mRelocaliser(mContext, mMap, mCamera)
//, mirSize(irVideoSize)
, mnInitialStage(TRAIL_TRACKING_NOT_STARTED)
, mlTrails()
, mFirstKF(mContext)
, mPreviousFrameKF(mContext)
{
...
void Tracker::TrackFrame(BasicImage<byte> &imFrame, bool bDraw)
{
    mbDraw = bDraw;
    //me:mMessageForUser.str("");   // Wipe the user message clean
    ...
    mbUseSBIInit = mContext->ptamTrackerUseRotationEstimator;
    
    if(!mpSBIThisFrame)
    {
        mpSBIThisFrame = new SmallBlurryImage(mCurrentKF, mContext->ptamTrackerRotationEstimatorBlur /**gvdSBIBlur*/);
        mpSBILastFrame = new SmallBlurryImage(mCurrentKF, mContext->ptamTrackerRotationEstimatorBlur /**gvdSBIBlur*/);
    }
    else
    {
    ...
    */
    ...
    if(mMap.IsGood())
    {
        if(mnLostFrames < 3)  // .. but only if we're not lost!
        {
            ...
            { // Provide some feedback for the user:
                //me:mMessageForUser << "Tracking Map, quality ";
                //me:if(mTrackingQuality == GOOD)  mMessageForUser << "good.";
                //me:if(mTrackingQuality == DODGY) mMessageForUser << "poor.";
                //me:if(mTrackingQuality == BAD)   mMessageForUser << "bad.";
                //me:mMessageForUser << " Found:";
                //me:for(int i=0; i<LEVELS; i++) mMessageForUser << " " << manMeasFound[i] << "/" << manMeasAttempted[i];
                //	    mMessageForUser << " Found " << mnMeasFound << " of " << mnMeasAttempted <<". (";
                //me:mMessageForUser << " Map: " << mMap.vpPoints.size() << "P, " << mMap.vpKeyFrames.size() << "KF";
            }
            ...
            {
                //me:mMessageForUser << " Adding key-frame.";
                AddNewKeyFrame();
            };
        }
        else  // what if there is a map, but tracking has been lost?
        {
            //me:mMessageForUser << "** Attempting recovery **.";
            if(AttemptRecovery())
            {
                TrackMap();
    ...
    {
        if(mContext->ptamTrackerAutoInit)
        {
            ...
            if(mnInitialStage == TRAIL_TRACKING_STARTED)
            {
                ...
                SmallBlurryImage sbiSecond(mCurrentKF);
                Vector<3> rotvec = CalcSBIRotation(mFirstKF.pSBI, &sbiSecond);
                ...
                for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end();++i)
                {
                    ...
                    Vector<2> firstvec = LevelZeroPos(trail.irInitialPos, mContext->ptamTrackerInitLevel);
                    Vector<2> z1vec =  mCamera.UnProject(LevelZeroPos(trail.irCurrentPos, mContext->ptamTrackerInitLevel));
                ...
                if(k>0)	// otherwise "if (mediandist>pPars.AutoInitPixel)" segfaults...
                {
                    ...
                    if(mediandist>mContext->ptamTrackerAutoInitPixel)
                        mbUserPressedSpacebar = true;
                }
            }
        }
...
std::vector<CVD::ImageRef> const& Tracker::GetTrackingCorners() const
{
    return mCurrentKF.aLevels[0].vCorners;
}

std::list<Trail> const& Tracker::GetTrackingTrails() const
{
    return mlTrails;
}
...
void Tracker::TrackForInitialMap()
{
    ...
    MiniPatch::mnMaxSSD = mContext->ptamTrackerMiniPatchMaxSSD;//*gvnMaxSSD;
    ...
    if(mnInitialStage == TRAIL_TRACKING_STARTED)
    {
        ...
        {
            mbUserPressedSpacebar = false;
            std::vector<std::pair<ImageRef, ImageRef> > vMatches;   // This is the format the mapmaker wants for the stereo pairs
...
void Tracker::TrailTracking_Start()
{
    ...
    int nToAdd = mContext->ptamTrackerMaxInitialTrails;
...
void Tracker::TrackMap()
{
    ...
    unsigned int nCoarseMax = mContext->ptamTrackerCoarseMax; //*gvnCoarseMax;
    unsigned int nCoarseRange = mContext->ptamTrackerCoarseRange; //*gvnCoarseRange;
    ...
    if(mContext->ptamTrackerDisableCoarse || mdMSDScaledVelocityMagnitude < mContext->ptamTrackerCoarseMinVelocity || nCoarseMax == 0)
        bTryCoarse = false;
    ...
    if(bTryCoarse && avPVS[LEVELS-1].size() + avPVS[LEVELS-2].size() > mContext->ptamTrackerCoarseMin)
    {
        ...
        unsigned int nFound = SearchForPoints(vNextToSearch, nCoarseRange, mContext->ptamTrackerCoarseSubPixIts /**gvnCoarseSubPixIts*/);
        vIterationSet = vNextToSearch;  // Copy over into the to-be-optimised list.
        if(nFound >= mContext->ptamTrackerCoarseMin /**gvnCoarseMin*/)  // Were enough found to do any meaningful optimisation?
        {
            mbDidCoarse = true;
    ...
    int nFinePatchesToUse = mContext->ptamTrackerMaxPatchesPerFrame - vIterationSet.size();
...
Vector<6> Tracker::CalcPoseUpdate(vector<TrackerData*> vTD, double dOverrideSigma, bool bMarkOutliers)
{
    ...
    */
    ...
    {
        if (mContext->ptamTrackerMEstimator == kMEstimatorTukey /*nEstimator == 0*/)
            dSigmaSquared = Tukey::FindSigmaSquared(vdErrorSquared);
        else if(mContext->ptamTrackerMEstimator == kMEstimatorCauchy /*nEstimator == 1*/)
...
void Tracker::AssessTrackingQuality()
{
    ...
    else
    {
        ...
        if(dTotalFracFound > mContext->ptamTrackerTrackingQualityGood /**gvdQualityGood*/)
            mTrackingQuality = GOOD;
        else if(dLargeFracFound < mContext->ptamTrackerTrackingQualityLost /**gvdQualityLost*/)
            mTrackingQuality = BAD;
        else
