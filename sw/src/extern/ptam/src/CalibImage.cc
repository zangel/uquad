...
CalibImage::CalibImage(Context *ctx)
: mContext(ctx)
{
}
...
bool CalibImage::MakeFromImage(Image<byte> &im)
{
    ...
    static int gvnCornerPatchSize = mContext->ptamCalibratorCornerPatchPixelSize;
    mvCorners.clear();
    mvGridCorners.clear();
    ...
    // and run the corner finding on that.
    {
        ...
        convolveGaussian(imBlurred, mContext->ptamCalibratorBlurSigma);
        ...
        int nGate = mContext->ptamCalibratorMeanGate;
        do
    ...
    if((int) mvCorners.size() < mContext->ptamCalibratorMinCornersForGrabbedImage)
        return false;
...
void CalibImage::ExpandByStep(int n)
{
    ...
    //static gvar3<int> gvnCornerPatchSize("CameraCalibrator.CornerPatchPixelSize", 20, SILENT);
    static double gvdMaxStepDistFraction = mContext->ptamCalibratorExpandByStepMaxDistFrac;
    static int gvnCornerPatchSize = mContext->ptamCalibratorCornerPatchPixelSize;
