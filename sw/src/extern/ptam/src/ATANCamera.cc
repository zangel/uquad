...
ATANCamera::ATANCamera(Context *ctx, string sName)
: mContext(ctx)
{
...
void ATANCamera::RefreshParams()
{
    ...
    mvFocal[0] = mvImageSize[0] * mContext->ptamCameraIntrinsicParameters[0];
    mvFocal[1] = mvImageSize[1] * mContext->ptamCameraIntrinsicParameters[1];
    mvCenter[0] = mvImageSize[0] * mContext->ptamCameraIntrinsicParameters[2] - 0.5;
    mvCenter[1] = mvImageSize[1] * mContext->ptamCameraIntrinsicParameters[3] - 0.5;
    ...
    mdW =  mContext->ptamCameraIntrinsicParameters[4];
    if(mdW != 0.0)
    {
    ...
    Vector<2> v2;
    v2[0]= max(mContext->ptamCameraIntrinsicParameters[2], 1.0 - mContext->ptamCameraIntrinsicParameters[2]) / mContext->ptamCameraIntrinsicParameters[0];
    v2[1]= max(mContext->ptamCameraIntrinsicParameters[3], 1.0 - mContext->ptamCameraIntrinsicParameters[3]) / mContext->ptamCameraIntrinsicParameters[1];
...
Matrix<2,NUMTRACKERCAMPARAMETERS> ATANCamera::GetCameraParameterDerivs()
{
    ...
    Vector<NUMTRACKERCAMPARAMETERS> vNNormal = mContext->ptamCameraIntrinsicParameters;
    ...
    for(int i=0; i<NUMTRACKERCAMPARAMETERS; i++)
    {
        ...
        mContext->ptamCameraIntrinsicParameters = vNNormal;
        RefreshParams();
    }
    if(mdW == 0.0)
...
void ATANCamera::UpdateParams(Vector<5> vUpdate)
{
    ...
    mContext->ptamCameraIntrinsicParameters = mContext->ptamCameraIntrinsicParameters + vUpdate;
    RefreshParams();
}
...
void ATANCamera::DisableRadialDistortion()
{
    ...
    mContext->ptamCameraIntrinsicParameters[NUMTRACKERCAMPARAMETERS-1] = 0.0;
    RefreshParams();
}
...
Vector<2> ATANCamera::UFBProject(const Vector<2>& vCam)
{
    ...
    mvLastIm[0] = mContext->ptamCameraIntrinsicParameters[2]  + mContext->ptamCameraIntrinsicParameters[0] * mvLastDistCam[0];
    mvLastIm[1] = mContext->ptamCameraIntrinsicParameters[3]  + mContext->ptamCameraIntrinsicParameters[1] * mvLastDistCam[1];
    return mvLastIm;
}
...
{
    mvLastIm = v2Im;
    mvLastDistCam[0] = (mvLastIm[0] - mContext->ptamCameraIntrinsicParameters[2]) / mContext->ptamCameraIntrinsicParameters[0];
    mvLastDistCam[1] = (mvLastIm[1] - mContext->ptamCameraIntrinsicParameters[3]) / mContext->ptamCameraIntrinsicParameters[1];
    mdLastDistR = sqrt(mvLastDistCam * mvLastDistCam);
    mdLastR = invrtrans(mdLastDistR);
    double dFactor;
    if(mdLastDistR > 0.01)
