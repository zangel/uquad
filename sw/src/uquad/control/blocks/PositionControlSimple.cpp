...
namespace blocks
{
    namespace
    {
        float scale_control(float ctl, float end, float dz, float dy)
        {
            if(ctl > dz)
            {
    ...
    PositionControlSimple::PositionControlSimple()
        ...
        , m_ParamVelocityP(0.08f, 0.08f, 1.0f)
        , m_ParamVelocityI(0.02f, 0.02f, 0.0f)
    ...
    {
        //m_ISDirectionControl.m_Value = Vec3f(0.0f, 0.0f, 0.0f);
        
        m_VelocityLP.update(-m_ISVelocityEstimated.m_Value, m_ISDT.m_Value);
        m_AttitudeMat = m_ISAttitudeEstimated.m_Value.toRotationMatrix();
        ...
        m_OSThrustSetpoint.m_Value = m_ThrustAbs;
        m_OSAttitudeSetpoint.m_Value = Quaternionf(m_AttitudeSPMat);
    ...
    void PositionControlSimple::resetPositionSP()
    {
        ...
        {
            m_bResetPositionSP = false;
            m_PositionSP(0) = m_ISPositionEstimated.m_Value(0);
            m_PositionSP(1) = m_ISPositionEstimated.m_Value(1);
        }
    }
    ...
    void PositionControlSimple::resetAltitudeSP()
    {
        ...
        {
            m_bResetAltitudeSP = false;
            m_PositionSP(2) = m_ISPositionEstimated.m_Value(2);
        }
    }
    ...
    void PositionControlSimple::directionControl()
    {
        Vec3f reqVelSP = Vec3f(
            m_ISDirectionControl.m_Value.x(),
            m_ISDirectionControl.m_Value.y(),
            #if 1
            scale_control(m_ISThrustControl.m_Value - 0.5f, 0.5f, 0.02f, 0.0f)
            #else
            m_ISThrustControl.m_Value
            #endif
        );
            ...
            std::abs(reqVelSP(1)) < m_ParamHoldXYThreshold )
        {
			if(!m_bPositionHoldEngaged)
            {
                if( m_ParamHoldXYMax < std::numeric_limits<float>::epsilon() ||
                    (
                        std::abs(m_ISVelocityEstimated.m_Value(0)) < m_ParamHoldXYMax &&
                        std::abs(m_ISVelocityEstimated.m_Value(1)) < m_ParamHoldXYMax
                    ) )
                {
		...
		if(std::abs(reqVelSP(2)) < std::numeric_limits<float>::epsilon())
        {
			if(!m_bAltitudeHoldEngaged)
            {
				if( m_ParamHoldZMax < std::numeric_limits<float>::epsilon() ||
                    std::abs(m_ISVelocityEstimated.m_Value(2)) < m_ParamHoldZMax )
                {
        ...
        float const yawRateMaxRad = m_ParamYawRateMax * piOver180;
        float const yawOffsetMax = yawRateMaxRad / m_ParamYawP;
        float const yawSPMoveRate = -m_ISDirectionControl.m_Value(2) * yawRateMaxRad;
        float yawOffset = yawSPMoveRate * m_ISDT.m_Value;
    ...
    void PositionControlSimple::thrustControl()
    {
        ...
        {
            m_VelocitySP(0) = (m_PositionSP(0) - m_ISPositionEstimated.m_Value(0)) * m_ParamPositionP(0);
            m_VelocitySP(1) = (m_PositionSP(1) - m_ISPositionEstimated.m_Value(1)) * m_ParamPositionP(1);
        }
        
        if(m_bRunAltitudeControl)
        {
            m_VelocitySP(2) = (m_PositionSP(2) - m_ISPositionEstimated.m_Value(2)) * m_ParamPositionP(2);
        }
        ...
        m_VelocityError = m_VelocitySP - m_ISVelocityEstimated.m_Value;
        
        #if 0
        m_VelocityError(0) = 0.0f;
        m_VelocityError(1) = 0.0f;
        #endif
        ...
        if(m_ThrustAbs > m_ParamThrustMax)
        {
            ...
            {
                float const k = m_ParamThrustMax / m_ThrustAbs;
                m_ThrustSP *= k;
                thrustSatXY = true;
                thrustSatZ = true;
            }
        ...
        if(!thrustSatZ)
        {
            m_ThrustInt(2) += m_VelocityError(2) * m_ParamVelocityI(2) * m_ISDT.m_Value;
