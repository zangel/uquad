...
namespace blocks
{
    UQuadSensors::UQuadSensors()
        ...
        , m_BarometerAltitudeLP(1.0f, 0.0f)
    ...
    system::error_code UQuadSensors::update(asio::yield_context yctx)
    {
        ...
        m_OSTime.m_Value = chrono::duration_cast<chrono::microseconds>(absT).count() * 1.0e-6f;
        m_OSDT.m_Value = chrono::duration_cast<chrono::microseconds>(dT).count() * 1.0e-6f;
        
        m_OSVelocityRate.m_Value = m_AccelerometerCalibration * m_SensorData.velocityRate;
        m_OSRotationRate.m_Value = m_GyroscopeCalibration * m_SensorData.rotationRate;
        m_OSMagneticField.m_Value = m_MagnetometerCalibration * m_SensorData.magneticField;
        ...
        m_OSPressure.m_Value = m_SensorData.baroPressure;
        ...
        m_BarometerAltitudeLP.update(153.8462f * baroTemperature * (1.0f - std::exp(0.190259f * std::log(baroPressureScaling))), m_OSDT.m_Value);
        m_OSAltitude.m_Value = m_BarometerAltitudeLP.state();
        //m_OSAltitude.m_Value = 153.8462f * baroTemperature * (1.0f - std::exp(0.190259f * std::log(baroPressureScaling)));
        
        m_OSTemperature.m_Value = m_SensorData.baroTemperature;
